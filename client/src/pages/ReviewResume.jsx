...
const ReviewResume = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    const base64Resume = await toBase64(input);

      const {data} = await axios.post('/api/ai/resume-review',{resume: base64Resume},{
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200 flex flex-col min-h-96 max-h-[600px]'>
          <div className='flex items-center gap-3'>
            ...
            <h1 className='text-xl flex justify-center items-center'>Analysis Results</h1>
          </div>
          {
            !content ? (
