...
const GenerateImages = () => {
  ...
  const ImageStyle = ["Realistic","Ghibli style","Anime style","Cartoon style","Fansty style","Realistic style","3D style","Portrait style"]
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <p className='mt-6 text-sm font-medium'></p>Describe your Image
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {
            ImageStyle.map((item)=>(
              <span onClick={()=>setSelectedStyle(item)} key={item} 
                  className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-purple-50 text-green-500':'text-gray-500 border-gray-300'}`}>
                {item}
              </span>
            ))
          }
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200 flex flex-col min-h-96'>
          <div className='flex items-center gap-3'>
            ...
            <h1 className='text-xl flex justify-center items-center'>Generated Images</h1>
          </div>
          { !content ? (
            <div className='flex-1 flex justify-center items-center'>
              <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
                <Image className='w-9 h-9'/>
                <p>Enter a description about the image and click "Generate Image" to get started</p>
              </div>
            </div>
          ) : (
